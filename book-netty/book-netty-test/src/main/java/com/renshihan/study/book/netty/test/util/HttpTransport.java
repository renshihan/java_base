...
public class HttpTransport {
     ...
     * @throws KeyManagementException
     */
    public Map<String, String> submit_https_Map(Object obj)
            throws CommunicationException, NoSuchAlgorithmException,
        ...
        if (obj != null) {
            if (obj instanceof Map) {
                ...
                try {
                    ...
                    if (logger.isErrorEnabled()) {
                        logger.error("Fatal transport error while try connect to ["
                                + url + "]");
                        logger.error("Cause: " + e.getMessage());
                    }
                    logger.error("HttpTransport Exception", e);
        ...
        try {
            ...
            throw new RuntimeException("解析返回数据为map失败", e);
        ...
        if (statusCode != HttpStatus.SC_OK) {
            if (logger.isErrorEnabled()) {
                logger.error("Answer from [" + url + "] status code: ["
                        + statusCode + "]");
            }
            throw new CommunicationException(String.valueOf(statusCode));
        }
